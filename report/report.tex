...
the reference wave is already in phase with the object wave, so no adjustment would be necessary. Just like the earlier section, the cosine identity in the top of the last equation (the \(\phi\) can be recongnized as any extra phase that came from the \(\psi_{o}\) \(\psi_{r}\) pair not counting the angle \(\theta\)), specifically, \(f_{x} = \frac{sin(\theta)}{\lambda}\).
